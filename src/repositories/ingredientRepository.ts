...
const ingredientRepo: Repository<Ingredient> =
...
export const getPaginatedIngredients = async (
  ...
  const [data, total] = await ingredientRepo.findAndCount({
    skip: (page - 1) * limit,
    take: limit,
    order: {
      Ing_name: "ASC",
...
export const findIngredientByBarcode = async (
  barcode: string
): Promise<Ingredient | null> => {
...
export const ingredientExists = async (
  name: string,
  brand?: string,
  barcode?: string
): Promise<boolean> => {
  const count = await ingredientRepo.count({
    where: [{ Ing_barcode: name, Ing_name: brand }, { Ing_barcode: barcode }],
  });
